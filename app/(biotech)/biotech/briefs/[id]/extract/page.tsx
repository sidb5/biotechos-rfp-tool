...
const TAG_STYLE: Record<FieldTag, string> = {
  STATED:   'bg-green-50 text-green-700 border-green-200',
...
export default function ExtractPage() {
  ...
  if (status === 'error') {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center gap-4 text-gray-700 px-6">
        ...
        <a href={`/biotech/briefs/new?id=${briefId}`} className="text-xs text-gray-500 hover:text-gray-700 transition-colors">
          ← Back to brief
        </a>
      </div>
    );
      ...
      <div className="mx-auto max-w-3xl px-5 py-10 space-y-8">
        ...
        <header>
          <nav className="mb-1.5 text-xs text-gray-500">
            ...
            <a href={`/biotech/briefs/new?id=${briefId}`} className="hover:text-gray-700 transition-colors">Input</a>
            <span className="mx-1.5">/</span>
            <span className="text-gray-500">Review extraction</span>
          </nav>
          <div className="flex items-start justify-between gap-4">
            ...
            <button
              onClick={runExtraction}
              className="shrink-0 text-xs text-gray-500 hover:text-gray-700 border border-gray-200 hover:border-gray-400 rounded-lg px-3 py-1.5 transition-colors"
            >
              ↺ Re-extract
            </button>
          </div>
        ...
        <section className="space-y-1.5">
          <label className="block text-sm font-medium text-gray-700">
            ...
            <span className="ml-2 text-xs font-normal text-gray-500">Internal name for this study — not shared with CROs</span>
          </label>
          <input
            type="text"
            value={title}
        ...
        <div className="flex flex-wrap items-center gap-3 text-xs">
          ...
          <span className="text-gray-500">— click any tag to cycle it</span>
        </div>
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <span className="ml-2 text-xs text-gray-500">{meta.hint}</span>
                  </div>
